...
- Snow `snow_imp`, Snow `frosty`, Freshwater `swamp_monster`, Terra `sapling`, Corruptus `raise_dead`, Void `void_spawn`, Scarak `scarak_egg`, Scarak `locust_queen`: summons must visibly fight for the caster, not merely spawn.
...
2. State the intended visual/mechanical contract.
3. Implement only the mismatch.
4. Verify with log proof plus screenshot/video when visual correctness matters.
