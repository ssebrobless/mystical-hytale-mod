...
public final class SpellbookInputHandler {
    ...
    public void handleMouseButton(PlayerMouseButtonEvent event) {
        try {
            ...
            if (event.getMouseButton().mouseButtonType != MouseButtonType.Left
                    || support.isDevBookItemId(itemId)
                    || eventItemInHand == null
                    || eventItemInHand.getWeapon() == null
                    || event.getTargetEntity() == null
                    || event.getTargetEntity().getReference() == null) {
                return;
            }

            send(player, support.handleWeaponFollowUpHit(
                    player,
                    playerData,
    ...
    private static void send(Player player, String response) {
        if (player != null && response != null && !response.isBlank()) {
