...
public final class PlayerMaintenanceRuntimeTaskProcessor implements RuntimeTaskProcessor {
    ...
    public void process(Store<EntityStore> currentStore) {
        ...
        processRuntimeEntityEffectClears(currentStore);
        processRuntimeRebuilds(currentStore);
        processProgressionBonusRefreshes(currentStore);
    }

    private void processRuntimeEntityEffectClears(Store<EntityStore> currentStore) {
        for (String playerId : tasks.pendingRuntimeEntityEffectClears()) {
            Player player = hooks.runtimePlayer(playerId);
            if (player == null) {
                tasks.recordTaskSkipped("runtime-entity-effect-clear", playerId, Map.of("reason", "player_unavailable"));
                tasks.completeRuntimeEntityEffectClear(playerId);
                continue;
            }
            if (!hooks.isPlayerInStore(player, currentStore)) {
                tasks.recordTaskSkipped("runtime-entity-effect-clear", playerId, Map.of("reason", "wrong_store"));
                continue;
            }

            String result = hooks.clearRuntimeEntityEffectsForDev(playerId, currentStore);
            tasks.recordTaskExecuted("runtime-entity-effect-clear", playerId, Map.of("result", result));
            tasks.completeRuntimeEntityEffectClear(playerId);
        }
    }
    ...
    public interface Hooks {
        ...
        String clearRuntimeEntityEffectsForDev(String playerId, Store<EntityStore> currentStore);
    }
}
